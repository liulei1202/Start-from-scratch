...
<html>
  <head>
	...
	function ImportFile() {
		...
		for ( var i = 0; i < rowsLength; i++) {
			...
			param = "DataImport/fileAutoImport.jsp?path=" + fileName;
  ...
  <body onload="setupUpload();" style="padding:30px 0 0 15px;">
	...
	sql = "select businessType,businessName,station,monitor,monitorlocation from "+table+" where bh = '"+bh+"'";
	...
	if(emcdb.dynaRs.next()){
   	%>
	   	...
	   	<table>
			...
			<td></td><td><input type="button" value="提交" id="import" onclick="javascript:modifyOriginalProcess('<%=bh%>','<%=type%>')"/></td>
			</tr>
		</table>
		</form>
   	<%}
