...
<html>
  <head>
	...
	function ImportFile() {
		...
		var fileName,hmlhttp;
		for ( var i = 0; i < rowsLength; i++) {
	
			fileName = infoTable.rows[i].cells[1].innerText;
			param = "&type=" + type + "&startTime=" + startTime +  "&endTime=" + endTime +  "&startFrequence=" + startFrequence +  "&endFrequence=" + endFrequence;
			...
			+"&longitude="+longitude+"&latitude="+latitude + "&bh=" + bh;
			...
			param = "DataImport/VideoFileAutoImport.jsp?fileName=" + fileName + param;
			...
			<div id="menu">
				...
				<%} %>
				  <script>
			       $("#CDB").click();
			       $("#CDBPDSM").click(); 	
			       $("#CDBPDSMDataimport").click();
			       $("#CDBPDSMVideoData").click();
            	  </script>
			</div>
			<div id="content">
			  ...
			  <h3>超短波-频段扫描-视频数据</h3>
				  <table id="info">
				  	<tr>
				  	<td>监测起始时间</td><td><input type="text" id="startTime" class="Wdate" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"></td>
				  	<td>监测终止时间</td><td><input type="text" id="endTime" class="Wdate" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})"></td>
				  	<td>中心频率(MHz)</td><td><input type="text" id="frequence" placeholder="单位：MHz"></td>
				  	</tr><tr>
				  ...
				  <input type="hidden" id="type" value="CDBPDSMVideoData">
