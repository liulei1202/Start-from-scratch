...
function fileQueued(file){
	addReadyFileInfo(file.id,file.name,"加载到上传队列");
}
...
function addFileMatchInfo(){
	...
	row.cells[5].innerHTML = "<font color='green'>nnnnn</font>";
}
...
function addReadyFileInfo(fileid,fileName,message,status){
	...
	col4.align = "right";
	col1.innerHTML = "<font size='2'>"+message+" : "+"</font>";
	...
	col4.innerHTML = "<a href='javascript:deleteFile(\""+fileid+"\")'><font size='2'>删除</font></a>";
}
...
function matchFile3(fileId){

	//alert("mathfile3");
	var infoTable = document.getElementById("infoTable");
	var row = document.getElementById(fileId);
	var filepath=row.cells[1].innerText;
	var state=row.cells[2].innerText;	
	//alert(state);
	if(state.indexOf("匹配成功")>-1){
		alert("文件已经导入数据库中！无需再次手动导入！");
		return;
	}
	if(state.indexOf("匹配失败")>-1){
		var url="../DataImport/Match.jsp?filepath="+filepath;
		window.open (url, 'newwindow','height=200, width=800,  toolbar=no, menubar=no, scrollbars=no, resizable=no,location=no, status=no') ;
