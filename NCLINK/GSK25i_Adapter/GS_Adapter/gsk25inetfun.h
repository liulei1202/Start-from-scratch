...
struct RUNSTAT_INFO
{
	...
	unsigned char gMode[NET_GGROUP_NUM];	//模态
	char reserver1;
	unsigned char gM[3];					//MMM
	//34
	int DspRunState;						//DSP运行状态
	unsigned short CurMoveCode;				//当前运行G代码
	unsigned short gS;						//编程主轴转速
	unsigned short gF;						//编程快速进给
	unsigned short HDT;						//当前刀
	unsigned short NXT;						//下一把刀
	unsigned short gH;						//H
	unsigned short gD;  					//D
	//52
    ...
    unsigned char undef_error;      //自定义的提示信息号
	char reserver[64];		//保留
};
...
功  能： 获取北航采集的系统运行信息
参  数： hInst:  GSK数据通信实例句柄
返回值：成功返回struct BHSAMPLE_STATIC指针, 否则返回NULL*/
struct BHSAMPLE_STATIC
{
	...
	short almflag;// 异常信号
	short reserved;
	//12+4*32+20+60+16*20+16*38+16*70+20
};

extern "C" GSK25I_API struct BHSAMPLE_STATIC* GSKRM_GetbhRunInfo(HINSGSKRM hInst);
